...
import {
  ArrowRight,
  Navigation2, Receipt, FileText, Smartphone
} from 'lucide-react';
...
function FeatureCard({ card, index }) {
  ...
  return (
    <motion.div
      className={`group relative bg-white/70 backdrop-blur-sm border border-border/60 rounded-2xl p-5 shadow-sm
    ...
    >
      {/* Top accent line */}
      <div className="absolute top-0 left-5 right-5 h-[2px] bg-gradient-to-r from-fg-green/0 via-fg-green/20 to-fg-green/0 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>

      {/* Card number + Icon row */}
      <div className="flex items-center justify-between mb-4">
        <span className="text-[10px] font-bold tracking-widest text-content-muted uppercase">{card.num}</span>
        <div className="w-8 h-8 rounded-lg bg-[#EAF5F0] border border-fg-green/10 flex items-center justify-center group-hover:bg-fg-green/10 transition-colors duration-300">
          <Icon className="w-4 h-4 text-fg-green" strokeWidth={1.5} />
        </div>
      </div>

      {/* Title */}
      <h3 className="text-[16px] font-semibold text-content tracking-tight mb-2 group-hover:text-fg-green transition-colors duration-300">
        {card.title}
      </h3>
...
function MobileFeatureCard({ card, index }) {
  ...
  return (
    <motion.div
      className="bg-white/70 backdrop-blur-sm border border-border/60 rounded-2xl p-5 shadow-sm"
    ...
    >
      <div className="flex items-center gap-3 mb-3">
        <div className="w-8 h-8 rounded-lg bg-[#EAF5F0] border border-fg-green/10 flex items-center justify-center shrink-0">
          <Icon className="w-4 h-4 text-fg-green" strokeWidth={1.5} />
        </div>
        <div>
          <span className="text-[10px] font-bold tracking-widest text-content-muted uppercase block leading-none mb-1">{card.num}</span>
          <h3 className="text-[15px] font-semibold text-content tracking-tight leading-tight">{card.title}</h3>
        </div>
      </div>
      <p className="text-[13px] text-content-secondary font-light leading-relaxed">
        {card.description}
...
export function IntelligenceModelSection() {
  return (
    <section id="how-vaahan-works" className="py-16 bg-[#fdfcfb] overflow-hidden relative scroll-mt-24">
      <div className="container mx-auto px-6 max-w-[1440px] relative z-10">
        <div className="flex flex-col xl:flex-row gap-12 xl:gap-16 items-center">

          {/* Left Column: Editorial (35%) */}
          <div className="w-full xl:w-[35%] flex flex-col justify-start relative z-50 shrink-0">
            <div>
              ...
              <h2 className="text-3xl md:text-4xl lg:text-[44px] font-medium tracking-tight leading-[1.05] mb-5 text-content">
                From data to<br/>
                <span className="text-fg-green">decisions.</span>
              </h2>

              <p className="text-[14px] text-content-secondary font-light leading-relaxed mb-6 max-w-sm">
          ...
          <div className="hidden md:block w-full xl:w-[65%]">
