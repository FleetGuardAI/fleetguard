...
def setup_test_companies():
    ...
    return comp_a.json()["token"]["access_token"], comp_b.json()["token"]["access_token"]


def test_cross_app_sync_and_isolation():
    token_a, token_b = setup_test_companies()
    ...
    # --- TEST 1: DRIVER -> OWNER ---
    # Driver logs in (simulate OTP login or just use admin token since it's testing API response)
    # Update trip status using Driver App API (which is just trip domain or driver mobile)
    # Note: In our current setup, driver_mobile endpoints modify duty status. Let's update the trip via general API.
