"""
FleetGuard Document Interpretation - Fitness Certificate
"""
...
from domain.document_interpretation.events import VehicleFitnessUpdated
from infrastructure.documents.models import StructuredDocument, DocumentFamily


class FitnessCertificateInterpreter(BaseDocumentInterpreter):
    @classmethod
    def key(cls) -> str:
        return "fitness_certificate_interpreter"

    @classmethod
    def get_business_type(cls) -> BusinessDocumentType:
        return BusinessDocumentType.FITNESS_CERTIFICATE
    ...
    def interpret(self, document: StructuredDocument) -> List[Any]:
        ...
        event = VehicleFitnessUpdated(
            source_document_id=str(document.document_id),
            fitness_certificate_number=cert_no,
            expiry_date=expiry
        )
        return [event]
