"""
Fleet Intelligence Engine - Decision Registry
"""
...
class DecisionRegistry:
    """
    Registry for dynamic registration and discovery of global decision engines.
    
    This registry is strictly responsible for type registration and lookup.
    It does NOT execute decision engines.
    """
    
    def __init__(self):
        self._registry: Dict[str, Type[BaseDecisionEngine]] = {}

    def register(self, engine_class: Type[BaseDecisionEngine]) -> None:
        """
        Registers a decision engine class by its stable key.
        """
        key = engine_class.key()
        if key in self._registry:
            raise ValueError(f"Decision engine '{key}' is already registered.")
            
        self._registry[key] = engine_class

    def get_engine(self, engine_key: str) -> Type[BaseDecisionEngine]:
        """
        ...
        if engine_key not in self._registry:
            raise ValueError(f"Decision engine '{engine_key}' is not registered.")
        return self._registry[engine_key]

    def enumerate_engines(self) -> List[Type[BaseDecisionEngine]]:
        """
